...
const CylindricalHeatTransfer = () => {
  const [layers, setLayers] = useState([{ k: "", r1: "", r2: "", l: "", material: "" }]);
  ...
  const addLayer = () => {
    setLayers([...layers, { k: "", r1: "", r2: "", l: "", material: "" }]);
  };
  ...
  const calculateResults = () => {
    ...
    layers.forEach((layer) => {
      ...
      if (k > 0 && r1 > 0 && r2 > r1 && l > 0) {
        totalResistance += Math.log(r2 / r1) / (2 * Math.PI * k * l);
      }
    });
    ...
    <Box sx={{ maxWidth: 600, margin: "50px auto", padding: "30px", borderRadius: "16px", boxShadow: "0 4px 10px rgba(0, 0, 0, 0.1)", backgroundColor: "#fff", textAlign: "center" }}>
      ...
      {layers.map((layer, index) => (
        <Box key={index} sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
          ...
          <TextField label="Comprimento (m)" type="number" value={layer.l} onChange={(e) => handleLayerChange(index, "l", e.target.value)} fullWidth />
